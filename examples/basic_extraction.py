...
Usage:
    python basic_extraction.py --video-id dQw4w9WgXcQ --method browser --interval 2 --frames 10
    python basic_extraction.py --video-id dQw4w9WgXcQ --method download --frame-rate 0.5
"""
...
def extract_frames_download(
    ...
    try:
        # Initialize the download extractor
...
def display_results(frames: List[Dict[str, Any]], video_id: str):
    """
    ...
    for i, frame_data in enumerate(frames[:5]):
        logger.info(f"Frame {i+1}: {frame_data['path']} (Time: {frame_data.get('time', 'N/A')}s)")
    
    if len(frames) > 5:
        logger.info(f"... and {len(frames) - 5} more frames")
    
    logger.info(f"Frames saved to {os.path.abspath(frames[0]['path']).rsplit(os.sep, 1)[0]}")

def main():
    """
    Main entry point for the basic extraction example.
    """
    parser = argparse.ArgumentParser(description="Extract frames from a YouTube video")
    ...
    # Extract frames using the selected method
